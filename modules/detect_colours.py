...
class DetectBlue:
    ...
    def run(self, image: str, output_path: Path, return_mask: bool = False) -> None | np.ndarray:
        ...
        lower_blue = np.array([92, 100, 0])
        upper_blue = np.array([132, 230, 255])
...
class DetectRed:
    ...
    def run(self, image: str, output_path: Path, return_mask: bool = False) -> None | np.ndarray:
        ...
        # Set upper and lower bounds for colour detection, this is in HSV
        lower_red_above = np.array([0, 100, 0])
        upper_red_above = np.array([10, 255, 255])

        lower_red_below = np.array([170, 100, 0])
        upper_red_below = np.array([180, 255, 255])

        # Apply the threshold for the colour detection
        mask_above = cv2.inRange(hsv, lower_red_above, upper_red_above)
        mask_below = cv2.inRange(hsv, lower_red_below, upper_red_below)
